...
function findWrappedHappyPathIf(body) {
  ...
  if (lastStatement === void 0 || lastStatement.type !== AST_NODE_TYPES.IfStatement) {
    return null;
  }
...
function negateTestExpression(sourceCode, test) {
  ...
  if (test.type === AST_NODE_TYPES.Identifier || test.type === AST_NODE_TYPES.MemberExpression || test.type === "ChainExpression") {
...
function needsParentheses(node) {
  return node.type === AST_NODE_TYPES.BinaryExpression || node.type === AST_NODE_TYPES.LogicalExpression || node.type === AST_NODE_TYPES.ConditionalExpression || node.type === AST_NODE_TYPES.AssignmentExpression;
}
